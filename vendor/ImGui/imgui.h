...
template<typename _Ty> void IM_DELETE(_Ty * p)   { if (p) { p->~_Ty(); ImGui::MemFree(p); } }
...
struct ImVector
{
    ...
    inline ImVector(const ImVector<_Ty>& src)                 { Size = Capacity = 0; Data = NULL; operator=(src); }
    inline ImVector<_Ty>& operator=(const ImVector<_Ty>& src)   { clear(); resize(src.Size); if (src.Data) memcpy(Data, src.Data, (size_t)Size * sizeof(_Ty)); return *this; }
    inline ~ImVector()                                      { if (Data) IM_FREE(Data); } // Important: does not destruct anything

    inline void         clear()                             { if (Data) { Size = Capacity = 0; IM_FREE(Data); Data = NULL; } }  // Important: does not destruct anything
    inline void         clear_delete()                      { for (int n = 0; n < Size; n++) IM_DELETE(Data[n]); clear(); }     // Important: never called automatically! always explicit.
    inline void         clear_destruct()                    { for (int n = 0; n < Size; n++) Data[n].~_Ty(); clear(); }           // Important: never called automatically! always explicit.

    inline bool         empty() const                       { return Size == 0; }
    inline int          size() const                        { return Size; }
    inline int          size_in_bytes() const               { return Size * (int)sizeof(_Ty); }
    inline int          max_size() const                    { return 0x7FFFFFFF / (int)sizeof(_Ty); }
    inline int          capacity() const                    { return Capacity; }
    inline _Ty&           operator[](int i)                   { IM_ASSERT(i >= 0 && i < Size); return Data[i]; }
    inline const _Ty&     operator[](int i) const             { IM_ASSERT(i >= 0 && i < Size); return Data[i]; }

    inline _Ty*           begin()                             { return Data; }
    inline const _Ty*     begin() const                       { return Data; }
    inline _Ty*           end()                               { return Data + Size; }
    inline const _Ty*     end() const                         { return Data + Size; }
    inline _Ty&           front()                             { IM_ASSERT(Size > 0); return Data[0]; }
    inline const _Ty&     front() const                       { IM_ASSERT(Size > 0); return Data[0]; }
    inline _Ty&           back()                              { IM_ASSERT(Size > 0); return Data[Size - 1]; }
    inline const _Ty&     back() const                        { IM_ASSERT(Size > 0); return Data[Size - 1]; }
    inline void         swap(ImVector<_Ty>& rhs)              { int rhs_size = rhs.Size; rhs.Size = Size; Size = rhs_size; int rhs_cap = rhs.Capacity; rhs.Capacity = Capacity; Capacity = rhs_cap; _Ty* rhs_data = rhs.Data; rhs.Data = Data; Data = rhs_data; }

    inline int          _grow_capacity(int sz) const        { int new_capacity = Capacity ? (Capacity + Capacity / 2) : 8; return new_capacity > sz ? new_capacity : sz; }
    inline void         resize(int new_size)                { if (new_size > Capacity) reserve(_grow_capacity(new_size)); Size = new_size; }
    inline void         resize(int new_size, const _Ty& v)    { if (new_size > Capacity) reserve(_grow_capacity(new_size)); if (new_size > Size) for (int n = Size; n < new_size; n++) memcpy(&Data[n], &v, sizeof(v)); Size = new_size; }
    inline void         shrink(int new_size)                { IM_ASSERT(new_size <= Size); Size = new_size; } // Resize a vector to a smaller size, guaranteed not to cause a reallocation
    inline void         reserve(int new_capacity)           { if (new_capacity <= Capacity) return; _Ty* new_data = (_Ty*)IM_ALLOC((size_t)new_capacity * sizeof(_Ty)); if (Data) { memcpy(new_data, Data, (size_t)Size * sizeof(_Ty)); IM_FREE(Data); } Data = new_data; Capacity = new_capacity; }
    inline void         reserve_discard(int new_capacity)   { if (new_capacity <= Capacity) return; if (Data) IM_FREE(Data); Data = (_Ty*)IM_ALLOC((size_t)new_capacity * sizeof(_Ty)); Capacity = new_capacity; }

    // NB: It is illegal to call push_back/push_front/insert with a reference pointing inside the ImVector data itself! e.g. v.push_back(v[10]) is forbidden.
    inline void         push_back(const _Ty& v)               { if (Size == Capacity) reserve(_grow_capacity(Size + 1)); memcpy(&Data[Size], &v, sizeof(v)); Size++; }
    inline void         pop_back()                          { IM_ASSERT(Size > 0); Size--; }
    inline void         push_front(const _Ty& v)              { if (Size == 0) push_back(v); else insert(Data, v); }
    inline _Ty*           erase(const _Ty* it)                  { IM_ASSERT(it >= Data && it < Data + Size); const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + 1, ((size_t)Size - (size_t)off - 1) * sizeof(_Ty)); Size--; return Data + off; }
    inline _Ty*           erase(const _Ty* it, const _Ty* it_last){ IM_ASSERT(it >= Data && it < Data + Size && it_last >= it && it_last <= Data + Size); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + count, ((size_t)Size - (size_t)off - (size_t)count) * sizeof(_Ty)); Size -= (int)count; return Data + off; }
    inline _Ty*           erase_unsorted(const _Ty* it)         { IM_ASSERT(it >= Data && it < Data + Size);  const ptrdiff_t off = it - Data; if (it < Data + Size - 1) memcpy(Data + off, Data + Size - 1, sizeof(_Ty)); Size--; return Data + off; }
    inline _Ty*           insert(const _Ty* it, const _Ty& v)     { IM_ASSERT(it >= Data && it <= Data + Size); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < (int)Size) memmove(Data + off + 1, Data + off, ((size_t)Size - (size_t)off) * sizeof(_Ty)); memcpy(&Data[off], &v, sizeof(v)); Size++; return Data + off; }
    inline bool         contains(const _Ty& v) const          { const _Ty* data = Data;  const _Ty* data_end = Data + Size; while (data < data_end) if (*data++ == v) return true; return false; }
    inline _Ty*           find(const _Ty& v)                    { _Ty* data = Data;  const _Ty* data_end = Data + Size; while (data < data_end) if (*data == v) break; else ++data; return data; }
    inline const _Ty*     find(const _Ty& v) const              { const _Ty* data = Data;  const _Ty* data_end = Data + Size; while (data < data_end) if (*data == v) break; else ++data; return data; }
    inline bool         find_erase(const _Ty& v)              { const _Ty* it = find(v); if (it < Data + Size) { erase(it); return true; } return false; }
    inline bool         find_erase_unsorted(const _Ty& v)     { const _Ty* it = find(v); if (it < Data + Size) { erase_unsorted(it); return true; } return false; }
    inline int          index_from_ptr(const _Ty* it) const   { IM_ASSERT(it >= Data && it < Data + Size); const ptrdiff_t off = it - Data; return (int)off; }
};
IM_MSVC_RUNTIME_CHECKS_RESTORE
