...
#define _IS_MEMFUNPTR(CALL_OPT, CV_OPT, REF_OPT, NOEXCEPT_OPT)                           \
    ...
    struct _Is_memfunptr<_Ret (CALL_OPT _Arg0::*)(_Types...) CV_OPT REF_OPT NOEXCEPT_OPT> \
        ...
        using _Bool_type = true_type;                                                     \
         typedef _Ret _RESULT_TYPE_NAME;                 \
...
//#undef _IS_MEMFUNPTR_ELLIPSIS
...
#ifdef __clang__
			template <class _Ty>
		inline constexpr bool is_member_function_pointer_v = __is_member_function_pointer(_Ty);
...
#ifdef __clang__
		template <class _Ty>
		inline constexpr bool is_member_object_pointer_v = __is_member_object_pointer(_Ty);
...
#ifdef __clang__
		template <class _Ty>
		inline constexpr bool is_member_pointer_v = __is_member_pointer(_Ty);
#else // ^^^ Clang / Other vvv
		template <class _Ty>
		...
		template <class _Ty>
		_CXX20_DEPRECATE_IS_POD inline constexpr bool is_pod_v = __is_pod(_Ty);
		...
		template <class _Ty1, class _Ty2>
		using _Conditional_type = decltype(false ?  declval<_Ty1>() :  declval<_Ty2>());
...
#pragma clang diagnostic ignored "-Wdeprecated-volatile"
		...
		inline constexpr bool _Is_nothrow_convertible_v = noexcept(_Implicitly_convert_to<_To>( declval<_From>()));
...
#pragma warning(pop)
		...
		using _Decltype_invoke_nonzero = decltype(_Invoker1<_Callable, _Ty1>::_Call(
			 declval<_Callable>(),  declval<_Ty1>(),  declval<_Types2>()...));
...
//#undef _FUNCTION_ARGS_ELLIPSIS
		...
		struct _Weak_result_type<_Ty, void_t<typename _Ty::result_type>> { // defined if _Ty::result_type exists
			 typedef typename _Ty::result_type _RESULT_TYPE_NAME;
		};
		_Pragma("warning(pop)")
		...
		struct _Weak_argument_type<_Ty, void_t<typename _Ty::argument_type>> : _Weak_result_type<_Ty> {
			// defined if _Ty::argument_type exists
			 typedef typename _Ty::argument_type _ARGUMENT_TYPE_NAME;
		};
		_Pragma("warning(pop)")
		...
		struct _Weak_binary_args<_Ty, void_t<typename _Ty::first_argument_type,
			...
			: _Weak_argument_type<_Ty> { // defined if both types exist
			 typedef typename _Ty::first_argument_type _FIRST_ARGUMENT_TYPE_NAME;
			 typedef typename _Ty::second_argument_type _SECOND_ARGUMENT_TYPE_NAME;
		};
		_Pragma("warning(pop)")
		...
		struct _Refwrap_has_ctor_from<_Ty, _Uty, void_t<decltype(_Refwrap_ctor_fun<_Ty>( declval<_Uty>()))>> : true_type {};
		...
		template <class _Ty1, class _Ty2>
		struct _Swappable_with_helper<_Ty1, _Ty2, void_t<decltype(swap( declval<_Ty1>(),  declval<_Ty2>()))>>
		...
		template <class _Ty1, class _Ty2>
		struct _Swap_cannot_throw : bool_constant<noexcept(swap( declval<_Ty1>(),  declval<_Ty2>())) //
			&& noexcept(swap( declval<_Ty2>(),  declval<_Ty1>()))> {
		...
		namespace _Has_ADL_swap_detail {
			...
			template <class _Ty>
			struct _Has_ADL_swap<_Ty, void_t<decltype(swap( declval<_Ty&>(),  declval<_Ty&>()))>> : true_type {};
		} // namespace _Has_ADL_swap_detail
...
#define _BITMASK_OPS(_BITMASK)                                                                                      \
    ...
    [[nodiscard]] constexpr _BITMASK operator~(_BITMASK _Left) noexcept { /* return ~_Left */                          \
        ...
        return static_cast<_BITMASK>(~static_cast<_IntTy>(_Left));                                                  \
                                                                                                                    ...
                                                                                                                    \
    [[nodiscard]] constexpr bool _Bitmask_includes(                                                                    \
        _BITMASK _Left, _BITMASK _Elements) noexcept { /* return (_Left & _Elements) != _BITMASK{} */               \
        return (_Left & _Elements) != _BITMASK{};                                                                   \
    }                                                                                                               \
                                                                                                                    \
    [[nodiscard]] constexpr bool _Bitmask_includes_all(                                                                \
		...
		struct _Conditionally_enabled_hash { // conditionally enabled hash base
			 typedef _Kty _ARGUMENT_TYPE_NAME;
		...
		template <>
		struct hash<float> {
			 typedef float _ARGUMENT_TYPE_NAME;
			 typedef size_t _RESULT_TYPE_NAME;
		...
		template <>
		struct hash<double> {
			 typedef double _ARGUMENT_TYPE_NAME;
			 typedef size_t _RESULT_TYPE_NAME;
		...
		struct hash<long double> {
			 typedef long double _ARGUMENT_TYPE_NAME;
			 typedef size_t _RESULT_TYPE_NAME;
		...
		struct hash<nullptr_t> {
			 typedef nullptr_t _ARGUMENT_TYPE_NAME;
			 typedef size_t _RESULT_TYPE_NAME;
		...
		template <class _Kty>
		struct _Is_nothrow_hashable < _Kty, void_t<decltype(hash<_Kty>{}( declval<const _Kty&>())) >>
			: bool_constant < noexcept(hash<_Kty>{}( declval<const _Kty&>())) > {};
