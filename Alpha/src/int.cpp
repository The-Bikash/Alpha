...
static constexpr const auto _zero = (dynamic_unsigned_int::_Mtype)(0);
static constexpr const auto _Hmax = UINT32_MAX;
static constexpr const auto _Tmax = ULLONG_MAX;
#endif


inline dynamic_unsigned_int::_Stype _Fndsiz(dynamic_unsigned_int::_Stype _Size) {
    dynamic_unsigned_int::_Stype _Tmp = 2;
    while (_Tmp < _Size)
        _Tmp <<= 1;
    return _Tmp;
}

inline void dynamic_unsigned_int::_Alloc(_Stype _Size)noexcept {
    _Size = _Fndsiz(_Size);
    auto _Ptr = iq::malloc(_Size << _shft);
    if (_Ptr == 0) {
        iq::fputs("Memory not available", stdout);
        iq::exit(1);
    }
    _Cap = _Size;
...
inline void dynamic_unsigned_int::_Calloc(_Stype _Size)noexcept {
    auto _Ptr = iq::calloc(_Size, _byte);
    if (_Ptr == 0) {
        iq::fputs("Memory not available", stdout);
        iq::exit(1);
    }
...
inline void dynamic_unsigned_int::_Rlloc(_Stype _Size) noexcept {
    if (_Size < _Siz) _Siz = _Size;
    _Size = _Fndsiz(_Size);
    auto _Tmp = iq::realloc(_Num, _Size << _shft);
...
inline void dynamic_unsigned_int::_Update(_Stype _Size)noexcept {
    if (_Size >= _Cap)
        _Rlloc(_Size + 1);
    iq::memset(_Num + _Siz, 0, (_Size - _Siz) << _shft);
    _Siz = _Size;
}

dynamic_unsigned_int::dynamic_unsigned_int()noexcept {
    _Calloc(2);
    _Siz = 1;
}

dynamic_unsigned_int::dynamic_unsigned_int(const initializer& _List)noexcept {
#if __special_compiler__
    _Siz = _List.size();
...
dynamic_unsigned_int::dynamic_unsigned_int(const _Stype _Size)noexcept {
    _Alloc(_Size);
    _Num[0] = 0;
    _Siz = 1;
}

dynamic_unsigned_int::dynamic_unsigned_int(const dynamic_unsigned_int& _That)noexcept {
    _Siz = _That._Siz;
    _Alloc(_That._Cap);
    iq::memcpy(_Num, _That._Num, _Siz << _shft);
}

dynamic_unsigned_int::dynamic_unsigned_int(dynamic_unsigned_int&& _That)noexcept {
    _Siz = _That._Siz;
    _Cap = _That._Cap;
    _Num = _That._Num;
...
constexpr dynamic_unsigned_int& dynamic_unsigned_int::operator=(const initializer& _That)noexcept {
...
constexpr dynamic_unsigned_int& dynamic_unsigned_int::operator=(const _Stype _Size)noexcept {
    if (_Size > _Cap) {
        iq::free(_Num);
        _Alloc(_Size);
    }
...
constexpr dynamic_unsigned_int& dynamic_unsigned_int::operator=(const dynamic_unsigned_int& _That)noexcept {
    if (this != &_That) {
        if (_That._Siz > _Cap) {
            iq::free(_Num);
            _Alloc(_That._Siz);
        ...
        _Siz = _That._Siz;
        iq::memcpy(_Num, _That._Num, _That._Siz << _shft);
    }
    return *this;
}

constexpr dynamic_unsigned_int& dynamic_unsigned_int::operator=(dynamic_unsigned_int&& _That)noexcept {
    if (this != &_That) {
        iq::free(_Num);
...
_NODISCARD constexpr iq::int01_t dynamic_unsigned_int::operator==(const dynamic_unsigned_int& _That)const noexcept {
    if (_Siz == _That._Siz) {
        auto _Ans = iq::memcmp(_Num, _That._Num, (size_t)_Siz << _shft);
        return _Ans == 0;
    }
    else return false;
}
...
constexpr dynamic_unsigned_int& dynamic_unsigned_int::operator<<=(const _Stype _Shift)noexcept {
    ...
    if (_Msz >= _Cap)
        _Rlloc(_Msz);
    if (_Sft) {
        iq::memmove(_Num + _Sft, _Num, _Siz << _shft);
        iq::memset(_Num, 0, _Sft << _shft);
        _Siz += _Sft;
    }
    if (_Rft) {
...
constexpr dynamic_unsigned_int& dynamic_unsigned_int::operator>>=(const _Stype _Shift)noexcept {
    ...
    if (_Sft) {
        ...
        _Siz -= _Sft;
        iq::memmove(_Num, _Num + _Sft, (size_t)_Siz << _shft);
    }
    if (_Rft) {
        auto _Rsift = _bits - _Rft;
        auto _Mask1 = _zero;
...
_NODISCARD dynamic_unsigned_int dynamic_unsigned_int::operator<<(const _Stype that)const noexcept {
    dynamic_unsigned_int _Tmp(*this);
    _Tmp <<= that;
    return _Tmp;
}
...
_NODISCARD dynamic_unsigned_int dynamic_unsigned_int::operator|(const dynamic_unsigned_int& _That)const noexcept {
    dynamic_unsigned_int _Tmp(*this);
    _Tmp |= _That;
    return _Tmp;
}
...
void _print(const dynamic_unsigned_int& _That)noexcept {
    char _Buff[_bits + 1]{};
    _Buff[_bits] = '\0';
    dynamic_unsigned_int::_Mtype _Mask = 0;
    dynamic_unsigned_int::_Mtype _Tmp = 1; _Tmp <<= (_bits - 1);
    for (dynamic_unsigned_int::_Type i = _That._Siz - 1; i >= 0; --i) {
        _Mask = _Tmp;
        for (auto j = 0; j < _bits; ++j) {
            _Buff[j] = (_That._Num[i] & _Mask) ? '1' : '0';
            _Mask >>= 1;
        }
        iq::fputs(_Buff, stdout);
    }
}

inline void add(dynamic_unsigned_int& _Answr, const dynamic_unsigned_int& _First, const dynamic_unsigned_int& _Secnd)noexcept {
    dynamic_unsigned_int::_Mtype* _This = 0;
    dynamic_unsigned_int::_Stype  _Siza = 0;
    dynamic_unsigned_int::_Mtype* _That = 0;
    dynamic_unsigned_int::_Stype  _Sizb = 0;
    ...
    for (dynamic_unsigned_int::_Stype i = 0; i < _Sizb; ++i) {
